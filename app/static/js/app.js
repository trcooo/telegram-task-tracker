...
function cleanTitle(txt){
  ...
  // Helper: remove tokens with unicode-friendly boundaries (spaces/punct)
  const rm = (pattern)=>{
    t = (" " + t + " ").replace(pattern, " ").replace(/\s+/g," ").trim();
  };

  // Commands
  rm(/(?:^|[\s\.,!?:;()])(добавь|добавить|создай|создать|запланируй|запланировать|поставь|поставить|сделай|напомни|напоминание)(?:$|[\s\.,!?:;()])/gi);

  // Relative dates
  rm(/(?:^|[\s\.,!?:;()])(на\s+)?(сегодня|завтра|послезавтра)(?:$|[\s\.,!?:;()])/gi);

  // Weekdays
  rm(/(?:^|[\s\.,!?:;()])(понедельник|вторник|среда|четверг|пятница|суббота|воскресенье|пн|вт|ср|чт|пт|сб|вс)(?:$|[\s\.,!?:;()])/gi);

  // Month names
  rm(/(?:^|[\s\.,!?:;()])(январ[ья]?|янв|феврал[ья]?|фев|март[а]?|мар|апрел[ья]?|апр|май[а]?|мая|июн[ья]?|июль|июл|июн|август[а]?|авг|сентябр[ья]?|сен|октябр[ья]?|окт|ноябр[ья]?|ноя|декабр[ья]?|дек)(?:$|[\s\.,!?:;()])/gi);

  // Dates numeric
  rm(/(?:^|[\s\.,!?:;()])([0-3]?\d)[.\-\/]([01]?\d)(?:[.\-\/]\d{2,4})?(?:$|[\s\.,!?:;()])/g);

  // Times HH:MM / HH.MM
  rm(/(?:^|[\s\.,!?:;()])([01]?\d|2[0-3])[:. ]([0-5]\d)(?:$|[\s\.,!?:;()])/g);

  // Plain hour numbers around time phrases
  rm(/(?:^|[\s\.,!?:;()])(\d{1,2})(?:$|[\s\.,!?:;()])/g);

  // Number words (hours)
  rm(/(?:^|[\s\.,!?:;()])(один|одного|два|двух|три|трех|трёх|четыре|четырех|четырёх|пять|пяти|шесть|шести|семь|семи|восемь|восьми|девять|девяти|десять|десяти|одиннадцать|одиннадцати|двенадцать|двенадцати|тринадцать|тринадцати|четырнадцать|четырнадцати|пятнадцать|пятнадцати|шестнадцать|шестнадцати|семнадцать|семнадцати|восемнадцать|восемнадцати|девятнадцать|девятнадцати|двадцать|двадцати|полдень|полночь|час)(?:$|[\s\.,!?:;()])/gi);

  // Durations words
  rm(/(?:^|[\s\.,!?:;()])(мин|минута|минуты|минут|час|часа|часов|полчаса|полтора)(?:$|[\s\.,!?:;()])/gi);

  // Prepositions/connectors that remain
  rm(/(?:^|[\s\.,!?:;()])(с|до|в|на)(?:$|[\s\.,!?:;()])/gi);
...
function parseVoiceCommand(rawText, baseIso, tz){
  ...
  // Strip leftover relative tokens (unicode-safe)
  cleaned = cleaned.replace(/после\s*-?\s*завтра/gi, 'послезавтра');
  cleaned = cleaned.replace(/(?:^|[\s\.,!?:;()])(на\s+)?(сегодня|завтра|послезавтра)(?:$|[\s\.,!?:;()])/gi, ' ');
  ...
  else if(mentionTimeLike) kind = "event";
