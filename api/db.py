...
def _migrate_schema() -> None:
    ...
    def _ensure_completed_default() -> None:
        ...
        # Best-effort: normalize legacy types to BOOLEAN so the ORM can safely write False/True.
        with engine.begin() as conn:
            if "int" in t:
                try:
                    conn.execute(text('ALTER TABLE "tasks" ALTER COLUMN "completed" TYPE BOOLEAN USING ("completed" <> 0)'))
                    t = "boolean"
                except Exception:
                    pass
                ...
                try:
                    conn.execute(
                        text(
                            'ALTER TABLE "tasks" ALTER COLUMN "completed" TYPE BOOLEAN '
                            'USING (lower(coalesce("completed"::text, \'\')) IN (\'true\',\'t\',\'1\',\'yes\',\'y\'))'
                        )
                    )
                    t = "boolean"
                except Exception:
        ...
        with engine.begin() as conn:
            ...
            try:
                ...
                pass
            # Ensure a server-side default so INSERTs that omit the column won't fail.
            try:
                conn.execute(text(f'ALTER TABLE "tasks" ALTER COLUMN "completed" SET DEFAULT {default_sql}'))
            except Exception:
                pass
            # Absolute fallback: if the column is stubbornly NOT NULL and no default could be applied,
            # drop NOT NULL so the app can function.
            try:
                conn.execute(text('ALTER TABLE "tasks" ALTER COLUMN "completed" DROP NOT NULL'))
            except Exception:
                pass
