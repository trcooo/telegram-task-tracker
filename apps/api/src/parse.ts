...
function extractListHint(text: string) {
  const m = text.match(/@([\p{L}0-9_\-]+)/u);
  if (!m) return { listHint: undefined, cleaned: text };
  const cleaned = text.replace(m[0], "").replace(/\s+/g," ").trim();
  return { listHint: m[1], cleaned };
}

export function parseNaturalInput(raw: string): Parsed {
  let text = raw.trim();
  const out: Parsed = { title: text };
  ...
  if (bangs.length >= 3) out.priority = 3;
  else if (bangs.length === 2) out.priority = 2;
  else if (bangs.length === 1) out.priority = 1;
  text = text.replace(/!+/g, "").trim();

  // kind keywords
  if (/\b(meet|meeting|встреч)/i.test(text)) out.kind = "meeting";
  else if (/\b(study|lesson|учеб|курс|лекц)/i.test(text)) out.kind = "study";

  const t = extractTags(text); out.tags = t.tags.length ? t.tags : undefined; text = t.cleaned;
  const l = extractListHint(text); out.listHint = l.listHint; text = l.cleaned;

  // date words
  let day = dayjs();
  if (tomorrowWords.test(text)) { day = day.add(1,"day"); text = text.replace(tomorrowWords,"").trim(); }
  if (todayWords.test(text)) { day = dayjs(); text = text.replace(todayWords,"").trim(); }
  ...
  if (rm) {
    ...
    if (tm) {
      const hh = Number(tm[1]); const mi = Number(tm[2]);
