...
async function resolveListId(userId: string, listHint?: string | null) {
  if (!listHint) return null;
...
async function replaceReminders(userId: string, taskId: string, reminders: Array<{ at: string; method?: string; status?: string }>) {
...
tasksRouter.get("/", async (req: AuthedRequest, res) => {
  ...
  const { view, date } = req.query as any;
...
tasksRouter.post("/parse", async (req: AuthedRequest, res) => {
...
tasksRouter.patch("/:id", async (req: AuthedRequest, res) => {
  ...
  const task = await prisma.task.update({
    where: { id, userId: req.userId! } as any,
    data: {
...
tasksRouter.delete("/:id", async (req: AuthedRequest, res) => {
  const id = req.params.id;
  await prisma.task.delete({ where: { id, userId: req.userId! } as any });
  res.json({ ok: true });
});
